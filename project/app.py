...
def films():
    ...
    genres = cursor.fetchall()
    cursor.execute('SELECT COUNT(t1.id) AS ct, t2.id FROM exam_reviews t1 JOIN exam_films t2 ON t1.film_id=t2.id WHERE t1.status_id IN (SELECT id FROM exam_reviews_status WHERE status="одобрена") GROUP BY t2.id;')
    reviews = cursor.fetchall()
    cursor.close()
...
def show(film_id):
    ...
    cursor.execute('SELECT t1.review_text, t1.rating, t1.date_added, t2.last_name, t2.first_name FROM exam_reviews t1 JOIN exam_users t2 ON t1.user_id=t2.id WHERE ((film_id=%s) and t1.status_id IN (SELECT id FROM exam_reviews_status WHERE status="одобрена"));', (film_id,))
    reviews = cursor.fetchall()
    cursor.execute('SELECT t3.name AS g_name, t1.id as f_id FROM exam_films t1 JOIN exam_films_genres t2 ON t1.id=t2.film_id JOIN exam_genres t3 ON t2.genre_id=t3.id ;')
    genres = cursor.fetchall()
    cursor.close()
